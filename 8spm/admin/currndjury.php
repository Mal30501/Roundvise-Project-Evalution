                ...
                //  $rownumimg=mysqli_fetch_array($data5) ;
                 if($rownum6){
                    while( $rownumpaper=mysqli_fetch_array($data6)){ 
                        $dataroundsel= mysqli_query($con,"SELECT * FROM submission where juryid= '$rownumpaper[uid]' and roundid=$rownum23[round_id]")or die(mysqli_error());
                        $dataroundselrow = mysqli_num_rows($dataroundsel); 
                        $rownumslernd=mysqli_fetch_array($dataroundsel);
                        $dataroundsel1= mysqli_query($con,"SELECT * FROM marks where juryid= '$rownumpaper[uid]' and roundmarks=$rownum23[round_id]")or die(mysqli_error());
